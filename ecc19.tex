...
choice. The simulation results are compared to a simple PI designed
...
continuous state spaces {[}\emph{doplnit van Hasselt}{]}. If the analytical
form of the Q function is unknown in continuous state spaces, it may
be represented by a universal function approximating method such as
Neural Network or Gaussian Process Regression (GPR).
...
points. Various sparse GPR \cite{ecc19ref:Bijl_Online_Sparse_Gaussian},\cite{ecc19ref:Candela_A_unifying_view},
\textit{{[}Huber}{]} techniques were developed to overcome this computational
...
their noisy measurements $y_{i}=f(\mathbf{x}_{i})+\boldsymbol{\varepsilon}_{i}$,
...
the covariances of all pairs of training and testing datasets, and
...
will be used later. Specifically, we remind that the term $\mathbf{G}=\mathbf{K}_{*f}\overline{\mathbf{K}}_{ff}^{-1}$
...
(predictions) $\mathbf{f}_{*}$ for any test points over a dataset
$\mathbf{X}_{*}$. Unfortunately, these simple calculations can get
very expensive due to the inverse of matrix \textbf{$\overline{\mathbf{K}}_{ff}$},
which is of size $n\times n$ where $n$ is the number of training
data points. This is the operation which costs $\mathcal{O}(n^{3})$.
This is the moment when sparse GPR is taken into account \cite{ecc19ref:Bijl_Online_Sparse_Gaussian}.
...
and MDP respectively. At a discrete time $k$, the vectors of states
$\mathbf{x}_{k}\in\mathcal{X}$ and inputs $\mathbf{u}_{k}\in\mathcal{U}$
are usually considered in CT for a process model, whereas MDP uses
...
in CT is an analogy of a deterministic policy $\mathbf{a}_{k}=\pi(\mathbf{s}_{k})$.
...
used in CT (not bounded in general, to be minimized, almost never
depending on $\mathbf{x}_{k+1}$). The ML theory will be discussed
below mostly with CT notations and assumptions.
...
It follows directly from (\ref{eq:q-split-sum}). For a policy $\pi$,
function $Q^{\pi}(x_{k},u_{k})$ must satisfy
...
It converges to the $Q^{*}$ provided the starting policy is stabilizing
in order to ensure the initial $Q^{\pi}$ is finite.
...
the process whereas $\mathbf{X}_{*}^{\pi}$ is a collection of states
observed as results of the actions accompanied with the actions the
evaluated strategy $\pi$ would presumably apply there. Note $\mathbf{X}_{*}^{\pi}$
...
$\mathbf{f}$ for the vector unknown function values used in GPR context
will be preserved
...
The noisy realization of $\mathbf{f}^{\pi}=\mathbb{\boldsymbol{\ell}+\gamma E}[Q^{\pi}(\mathbf{X}_{*}^{\pi})]$
is $\mathbf{y}^{\pi}=\mathbf{\boldsymbol{\ell}}+\gamma\mathbf{f}_{*}^{\pi}$.
...
the Q estimates depend on $\mathbf{f}_{*}^{\pi}$ value which we do
not know. Recall that Q function is neither measured nor observed.
However, consider the left hand side equals the true values \textbf{$\mathbf{f}^{\pi}$}
and $\mathbf{f}_{*}^{\pi}$ for $k\rightarrow\infty$ and sufficient
excitation in the state-action space, and when $\mathbf{m}=\mathbf{f}^{\pi}$
and $\mathbf{m}_{*}=\mathbf{f}_{*}^{\pi}$. The \textbf{$\mathbf{f}^{\pi}$}
and $\mathbf{f}_{*}^{\pi}$ thus represent a fixed point of the following
...
$\mathbf{m},\mathbf{m}_{*}$. Instead of actually iterating (\ref{eq:GPR-iterations}),
a system of linear equations the fixed point values satisfy can be
solved. However, this system of linear equations will typically be
ill-conditioned. In general, the update (\ref{eq:Q-data-update})
...
and some information from the starting values $\mathbf{m},\mathbf{m}_{*}$
does not not vanish. We propose to regularize this situation shifting
...
\noindent  Now imagine that also the Q function value at a general
query point $\mathbf{\mathit{f}}_{q}^{\pi}=Q^{\pi}(\mathbf{u}_{k}^{q},\mathbf{x}_{k})$
...
may now be obtained reusing the above estimates $\hat{\mathbf{f}}^{\pi},\mathbf{\hat{f}_{*}^{\pi}}$
as well as $\overline{\mathbf{K}}_{ff}^{-1}$ and recalculating only
a row kernel matrix $\mathbf{K}_{qf}^{\pi}$ which is the only datum
changed by the query point. The result is
...
In the first step, it calculates the matrix inversion $\overline{\mathbf{K}}_{ff}^{-1}$
for later uses as it depends only on the data, not the optimized policy.
$Q^{\pi^{(i)}}$ estimate is calculated for each policy $\pi^{(i)}$
starting with some known stabilizing policy $\pi^{(1)}$ solving a
linear system of equations (\ref{eq:Q-unbiased-estimate}). All available
data can be used in this step. This $Q^{\pi^{(i)}}$ is then minimized
at each state from $\mathbf{X}_{*}^{\pi}$ at \ref{enu:update-pi}
...
\begin{centering}
\fbox{\parbox[c]{3in}{%
\textbf{Require: $k([\mathbf{u},\mathbf{x}],[\mathbf{u}',\mathbf{x}']),\epsilon,\gamma,\xi,\mathbf{x}_{j},\mathbf{u}_{j},\boldsymbol{\ell}_{j},\pi^{(1)}$}

$1\leq j\leq k$
\begin{enumerate}
\item calculate kernel and its inversion $\overline{\mathbf{K}}_{ff}^{-1}$\label{enu:calculate-kernel-inv}
\item $i=1$,\textbf{ repeat}

\begin{enumerate}
\item update $\mathbf{X}_{*}^{\pi^{(i)}}$ and kernel $\mathbf{K}_{*f}^{(i)}$\label{enu:update-kalman-gain}
\item calculate $\hat{\mathbf{f}}^{(i)},\mathbf{\hat{f}}_{*}^{(i)}$ using
(\ref{eq:Q-unbiased-estimate})\label{enu:calculate-Q-estimate}
...
\begin{figure}
\centering{}\includegraphics[width=0.95\columnwidth,bb = 0 0 200 100, draft, type=eps]{training_data.eps}
...
\begin{figure}
\centering{}\includegraphics[width=0.95\columnwidth,bb = 0 0 200 100, draft, type=eps]{q_learning_improved.eps}
...
use more electricity. All this is modeled by the second term in $\ell$.
As such, the policy resembles a proportional feedback controller with
variable gain. It does not have any integral action whereas a proportional
derivative integral (PID) controller would be normally used for similar
...
A PI controller designed for a linearized model of FCU at $u_{0}=1,x_{0}=22\left[\unit{\lyxmathsym{\textdegree}C}\right]$
...
\begin{figure}
\centering{}\includegraphics[width=0.95\columnwidth,bb = 0 0 200 100, draft, type=eps]{q_learning_vs_pi_contr.eps}
...
\end{figure}
\begin{figure}
\centering{}\includegraphics[width=0.95\columnwidth,bb = 0 0 200 100, draft, type=eps]{optimal_policy.eps}
...
over $-7\pm\varDelta$. The result is shown in Fig. (\ref{fig:Q-Learning-PI-PI-noisy}).
...
\begin{figure}
\centering{}\includegraphics[width=0.95\columnwidth,bb = 0 0 200 100, draft, type=eps]{optimal_policy_noisy.eps}
...
the process. It is more accurate than just solving the so-called temporal
difference equation in the least squares sense which results in a
biased estimate in the presence of process noise \cite{ecc19ref:Bratke_Linear_Least_Squares_Algo}.
...
were compared in terms of cumulative loss $L$ towards $L^{*}$ found
by the Q-learning. The best PI controller from the grid was slightly
worse than $L^{*}$. However, such direct controller search is impractical
without a simulation model because it requires evaluating many controllers
from defined initial conditions and affected by defined disturbances.
Also, a PI controller was designed based on a linearized FCU model
and PI tuning rules. This traditional approach could actually be used
in practice together with, for example, Ziegler Nichols PID calibration
method. The controllers were compared in Section \ref{sec:RESULTS}
and the performance of PI designed using linearized FCU was shown
to be worse than both Q-Learning and the PI found by direct search.
Here the problem may be the choice of the linearization point. Although
the example was a single input single output control problem, the
method applies to multidimensional problems without modifications.
