...
their noisy measurements $\mathbf{y}_{i}=f(\mathbf{x}_{i})+\boldsymbol{\mathbf{\varepsilon}}_{i}$,
...
\[
\mathrm{cov}(\mathbf{y})=\kappa(\mathbf{X},\mathbf{X})+\sigma_{n}^{2}\mathbf{I}.
\]
...
\kappa(\mathbf{X},\mathbf{X})+\sigma_{n}^{2}\mathbf{I} & \kappa(\mathbf{X},\mathbf{X}_{*})\\
\kappa(\mathbf{X}_{*},\mathbf{X}) & \kappa(\mathbf{X}_{*},\mathbf{X}_{*})
...
mean and covariance, $\mathbf{K}_{f*}$ is a $n\times n_{*}$ matrix
of the covariances of all pairs of training and testing datasets,
and $\mathbf{K}_{ff}$, $\mathbf{K}_{**}$, $\mathbf{K}_{*f}$ analogously.
Let's also use a notation $\mathbf{\overline{K}}_{aa}=\mathbf{K}_{aa}+\sigma_{n}^{2}\mathbf{I}$
for any $a$. There are many useful covariance functions $\kappa(\mathbf{x},\mathbf{x}')$
called kernels, e.g. squared exponential (SE)

\[
\kappa(\mathbf{x},\mathbf{x}')=\sigma_{f}^{2}\exp\left(-\frac{1}{2l^{2}}\left|\mathbf{x}-\mathbf{x}'\right|^{2}\right)+\sigma_{n}^{2},
\]
...
\begin{eqnarray}
p(\mathbf{f}_{*}|\mathbf{y}) & = & \mathcal{N}(\boldsymbol{\mu}_{*},\mathbf{\Sigma}_{*}),\nonumber \\
...
\noindent where $\boldsymbol{\mu}_{*}$ is a mean vector and $\mathbf{\Sigma}_{*}$
...
then minimized at each state from $\mathbf{X}_{*}^{\pi}$ at \ref{enu:update-pi}
...
a subject of research. The approach was verified on a simple linear
model against Q function calculated as a solution of Riccati equation,
this verification is not part of the paper. It was then tested on
a simplified nonlinear one-input one-state FCU simulation model and
an optimal control policy $\pi^{*}$ was calculated. The result makes
sense intuitively, the feedback gain is gradually decreasing with
the control error. A grid of PI controllers were compared in terms
of cumulative loss $L$ towards $L^{*}$ found by the Q-learning.
